...
const PLAN_PATTERN = /(今天练什么|给我.*计划|冲分计划|学习计划|下一步练什么|今日计划|训练计划|今天该练什么|今天该怎么练|安排训练)/i;
...
export function shouldUsePracticeData({ intent, message, recentMessages = [] } = {}) {
  ...
  if (normalizedIntent === "data_analysis" || normalizedIntent === "plan") {
    return true;
  }
...
function resolveTemperature(intent, fallback = DEFAULT_TEMPERATURE) {
  ...
  if (normalizedIntent === "data_analysis" || normalizedIntent === "plan" || normalizedIntent === "continuation") {

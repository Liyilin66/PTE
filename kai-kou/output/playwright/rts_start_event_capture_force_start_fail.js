async (page) => {
  ...
  await page.addInitScript(() => {
    class ForcedStartFailMediaRecorder {
      ...
      addEventListener() {}
      removeEventListener() {}
      start() { throw new DOMException('forced_mediarecorder_start_fail', 'NotSupportedError'); }
      stop() { this.state = 'inactive'; if (typeof this.onstop === 'function') this.onstop(); }
      requestData() {}
    }
    try {
      Object.defineProperty(window, 'MediaRecorder', { value: ForcedStartFailMediaRecorder, configurable: true, writable: true });
    } catch {
      window.MediaRecorder = ForcedStartFailMediaRecorder;
    }
  });
  await page.addInitScript(() => {

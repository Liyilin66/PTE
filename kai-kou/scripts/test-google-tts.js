...
const OUTPUT_FILE_PATH = "D:/PTE/wfd/debug_tts/test.mp3";
const REQUEST_TIMEOUT_MS = Math.max(1, Number(process.env.WFD_TTS_TEST_TIMEOUT_MS) || 60000);
...
async function main() {
  ...
  logWithTimestamp(
    ...
    JSON.stringify({
      fallback: true,
      endpoint: ttsClient.config.endpoint
    })
  );

  const voice = {
    languageCode: process.env.WFD_TTS_TEST_LANGUAGE_CODE || "en-AU",
    name: process.env.WFD_TTS_TEST_VOICE_NAME || "en-AU-Standard-C"
  };
  const audioConfig = {
    audioEncoding: "MP3",
    speakingRate: 0.92
...
main().catch((error) => {
  console.error(`[${getTimestamp()}] test-google-tts failed`);
